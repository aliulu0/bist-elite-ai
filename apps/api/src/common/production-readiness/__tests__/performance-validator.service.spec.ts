...
jest.mock('../../monitoring/metrics.service', () => ({
  MetricsService: jest.fn().mockImplementation(() => ({
    getSnapshot: jest.fn().mockReturnValue({
      ...
      system: { cpuUsage: { user: 1000, system: 500 }, memoryUsage: process.memoryUsage(), uptime: 1000, activeHandles: 10, activeRequests: 2 },
...
describe('PerformanceValidatorService', () => {
  ...
  describe('validate', () => {
    ...
    it('should return WARN for poor performance', () => {
      const mockMetrics = {
        getSnapshot: jest.fn().mockReturnValue({
          ...
          system: { cpuUsage: { user: 1000, system: 500 }, memoryUsage: process.memoryUsage(), uptime: 1000, activeHandles: 10, activeRequests: 2 },
          database: { slowQueries: [{ query: 'test', duration: 2000, timestamp: Date.now() }], totalQueries: 50, failedQueries: 0 },
          worker: { activeJobs: 0, completedJobs: 10, failedJobs: 0, queueLength: 0 },
          timestamp: new Date().toISOString(),
        }),
        incrementCounter: jest.fn(),
    ...
    it('should respect custom thresholds', () => {
      ...
      const svc = new PerformanceValidatorService(mockMetrics as never, { 'api.avg': 30, 'api.p95': 50, 'api.p99': 100 });
      const result = svc.validate();
      expect(result.status).toBe(ReadinessStatus.WARN);
    });
  });

...
export class PerformanceValidatorService {
  ...
  validate(): PerformanceValidationResult {
    ...
    benchmarks.push({
      ...
      status:
        snapshot.requests.avgDuration <= this.thresholds['api.avg']
    ...
    benchmarks.push({
      ...
      status:
        snapshot.requests.p95Duration <= this.thresholds['api.p95']
          ? ReadinessStatus.PASS
          : ReadinessStatus.WARN,
    });
    ...
    benchmarks.push({
      ...
      status:
        snapshot.requests.p99Duration <= this.thresholds['api.p99']
          ? ReadinessStatus.PASS
          : ReadinessStatus.WARN,
    });
    ...
    if (slowQueryCount > 0) {
      benchmarks.push({
        ...
        status:
          slowQueryCount > this.thresholds['db_query.slow']
            ? ReadinessStatus.WARN
            : ReadinessStatus.PASS,
      });
    }

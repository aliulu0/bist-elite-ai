...
@Injectable()
export class GoogleNewsProvider extends BaseAiResearchProvider {
  constructor() {
    super(getProviderConfig('google-news'));
  }

  protected collectEvidence(bundle: ResearchBundle): AiEvidenceItem[] {
    return bundle.news
      .filter((article) => article.provider === 'google-news')
  ...
  private toEvidence(article: ResearchArticle): AiEvidenceItem {
    ...
    return {
      id: `gn-${this.hashId(article.url || article.id)}`,
      provider: this.name,

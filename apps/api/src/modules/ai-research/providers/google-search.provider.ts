...
@Injectable()
export class GoogleSearchProvider extends BaseAiResearchProvider {
  constructor() {
    super(getProviderConfig('google-search'));
  }

  protected collectEvidence(bundle: ResearchBundle): AiEvidenceItem[] {
    return bundle.news
      .filter((article) => article.provider === 'agent-reach')
  ...
  private toEvidence(article: ResearchArticle): AiEvidenceItem {
    ...
    return {
      id: `gs-${this.hashId(article.url || article.id)}`,
      provider: this.name,
      source: article.source || 'Google Search',
      sourceType: 'search',
      title: article.title,

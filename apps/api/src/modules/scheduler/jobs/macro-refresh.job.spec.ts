import { MacroRefreshJob } from './macro-refresh.job';
...
describe('MacroRefreshJob', () => {
  it('should execute successfully', async () => {
    const job = new MacroRefreshJob(makeMacroService());
    const result = await job.execute();
    expect(result.success).toBe(true);
    expect(result.message).toContain('Macro data refreshed');
    expect(result.metadata).toHaveProperty('sourceCount');
    expect(result.metadata).toHaveProperty('healthyCount');
    expect(result.metadata).toHaveProperty('regime');
    expect(result.metadata).toHaveProperty('macroScore');
  });

...
export class AnalysisService {
  ...
  constructor(
    ...
    @Optional()
    @Inject(FUNDAMENTAL_PROVIDER)
    private readonly fundamentalProvider?: IFundamentalProvider,
  ...
  async analyzeSymbol(symbol: string, timeframe: Timeframe): Promise<AnalysisResult> {
    ...
    if (this.persistenceService) {
      this.persistenceService.saveAnalysisResult({ result }).catch((err) => {
        this.logger.warn(
          `Failed to persist analysis result for ${cleanSymbol}: ${err instanceof Error ? err.message : String(err)}`,
        );
      });
    }
  ...
  async analyzeTechnical(symbol: string, timeframe: Timeframe): Promise<AnalysisResult> {
    ...
    return {
      ...
      confluence: {
        confluenceScore: 0,
        agreement: 'VERY_LOW',
        financialAlignment: { score: 0, direction: 'neutral', confidence: 0, factors: [] },
        technicalAlignment: { score: 0, direction: 'neutral', confidence: 0, factors: [] },
        smartMoneyAlignment: { score: 0, direction: 'neutral', confidence: 0, factors: [] },
        trendAlignment: { score: 0, direction: 'neutral', confidence: 0, factors: [] },
        confidence: 0,
        metadata: {},
        isValid: false,
      },
      ...
      eliteScore: {
        ...
        summary: '',
        breakdown: {
          financial: { score: 0, weight: 0, contribution: 0 },
          technical: { score: 0, weight: 0, contribution: 0 },
          opportunity: { score: 0, weight: 0, contribution: 0 },
          confluence: { score: 0, weight: 0, contribution: 0 },
          candidate: { score: 0, weight: 0, contribution: 0 },
        },
        metadata: {},
        isValid: false,
      },
  ...
  private async buildDataset(symbol: string, timeframe: Timeframe): Promise<HistoricalDataset> {
    ...
    return {
      ...
      metadata: {
        ...
        dateRange: { start: bars[0]?.timestamp ?? '', end: bars[bars.length - 1]?.timestamp ?? '' },
        normalizedFields: [],
        warnings: [],
        processedAt: new Date().toISOString(),
        sourceProviders: ['auto'],
      },
    };
  }

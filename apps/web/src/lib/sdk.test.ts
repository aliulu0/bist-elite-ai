...
describe('sdkClient', () => {
  ...
  it('technicalAnalysis calls correct endpoint', async () => {
    ...
    expect(mockFetch).toHaveBeenCalledWith(
      expect.stringContaining('/technical-analysis/GARAN'),
      expect.any(Object),
    );
  ...
  it('scanner calls correct endpoint', async () => {
    ...
    expect(res).toHaveProperty('toplamHisse');
  });
  ...
  it('financialRules calls correct endpoint', async () => {
    ...
    expect(mockFetch).toHaveBeenCalledWith(
      expect.stringContaining('/financial-analysis/AKBNK'),
      expect.any(Object),
    );
  ...
  it('configuration calls correct endpoint', async () => {
    setupFetch({
      success: true,
      data: { totalDomains: 0, domains: {}, version: 1 },
      timestamp: '',
    });
  ...
  it('performanceMonitor calls correct endpoint', async () => {
    setupFetch({
      success: true,
      data: { metrics: [], system: {}, cache: {}, health: {}, totalRecorded: 0 },
      timestamp: '',
    });
  ...
  it('providerHealth calls correct endpoint', async () => {
    setupFetch({
      success: true,
      data: { providers: [], overallStatus: 'healthy', totalProviders: 0 },
      timestamp: '',
    });
    const res = await sdkClient.providerHealth();
    expect(mockFetch).toHaveBeenCalledWith(
      expect.stringContaining('/providers/health'),
      expect.any(Object),
    );
    expect(res).toHaveProperty('data');
  });
  ...
  it('workflowQueue calls correct endpoint', async () => {
    ...
    expect(mockFetch).toHaveBeenCalledWith(
      expect.stringContaining('/v1/queue/jobs'),
      expect.any(Object),
    );
    expect(res).toHaveProperty('data');
  });
  ...
  it('portfolioReport calls correct endpoint', async () => {
    ...
    expect(mockFetch).toHaveBeenCalledWith(
      expect.stringContaining('/portfolio/pf-1/report'),
      expect.any(Object),
    );
    expect(res).toHaveProperty('data');
  });
  ...
  it('portfolioPositions calls correct endpoint', async () => {
    ...
    expect(mockFetch).toHaveBeenCalledWith(
      expect.stringContaining('/portfolio/pf-1/positions'),
      expect.any(Object),
    );
    expect(res).toHaveProperty('data');
  });
  ...
  it('portfolioExecuteTransaction posts to correct endpoint', async () => {
    setupFetch({ success: true, data: {}, timestamp: '' });
    const res = await sdkClient.portfolioExecuteTransaction('pf-1', {
      symbol: 'THYAO',
      type: 'BUY',
      quantity: 10,
      price: 300,
    ...
    expect(mockFetch).toHaveBeenCalledWith(
      expect.stringContaining('/portfolio/pf-1/transactions'),
      expect.objectContaining({ method: 'POST' }),
    );
    expect(res).toHaveProperty('data');
  });
  ...
  it('watchlistAdd posts to correct endpoint', async () => {
    ...
    expect(mockFetch).toHaveBeenCalledWith(
      expect.stringContaining('/watchlist/FAVORITES/THYAO'),
      expect.objectContaining({ method: 'POST' }),
    );
  ...
  it('alerts calls correct endpoint', async () => {
    setupFetch({
      success: true,
      data: { alerts: [], total: 0, limit: 50, offset: 0 },
      timestamp: '',
    });
    const res = await sdkClient.alerts();
    expect(mockFetch).toHaveBeenCalledWith(
      expect.stringContaining('/alerts?limit=50&offset=0'),
      expect.any(Object),
    );
    expect(res).toHaveProperty('data');
  });
  ...
  it('alertsDismiss posts to correct endpoint', async () => {
    ...
    expect(mockFetch).toHaveBeenCalledWith(
      expect.stringContaining('/alerts/alert-1/dismiss'),
      expect.objectContaining({ method: 'POST' }),
    );
    expect(res).toHaveProperty('message');
  });
  ...
  it('telegram status calls correct endpoint', async () => {
    ...
    expect(mockFetch).toHaveBeenCalledWith(
      expect.stringContaining('/telegram/status'),
      expect.any(Object),
    );
  ...
  it('telegram preview calls correct endpoint', async () => {
    ...
    expect(mockFetch).toHaveBeenCalledWith(
      expect.stringContaining('/telegram/preview'),
      expect.any(Object),
    );
  ...
  it('telegram send posts to correct endpoint', async () => {
    ...
    expect(mockFetch).toHaveBeenCalledWith(
      expect.stringContaining('/telegram/radar/send?dryRun=false'),
      expect.objectContaining({ method: 'POST' }),
    );
  ...
  it('telegram deliveries calls correct endpoint', async () => {
    ...
    expect(mockFetch).toHaveBeenCalledWith(
      expect.stringContaining('/telegram/deliveries?limit=50'),
      expect.any(Object),
    );

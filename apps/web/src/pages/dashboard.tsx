...
const expectedProviders = ['Yahoo', 'Investing', 'Fintables', 'TCMB', 'KAP', 'MKK'];
...
function toMarketWidgets(
  overview: MarketOverview | null,
  macroScore: number | null,
): MarketWidget[] {
  if (!overview || overview.bist100.value <= 0) {
    return [];
  }
  const lastUpdate = new Date().toLocaleTimeString('tr-TR', { hour: '2-digit', minute: '2-digit' });
  const widgets: MarketWidget[] = [
    {
      symbol: 'XU100',
      label: 'BIST100 Eşdeğeri',
      value: overview.bist100.value.toLocaleString('tr-TR', { maximumFractionDigits: 2 }),
      change: overview.bist100.changePercent,
      trend:
        overview.bist100.changePercent > 0
          ? 'up'
          : overview.bist100.changePercent < 0
            ? 'down'
            : 'flat',
  ...
  for (const sector of overview.sectorHeatmap.slice(0, 4)) {
    widgets.push({
      symbol: sector.sector,
      label: sector.sector,
      value: `${sector.changePercent >= 0 ? '+' : ''}${sector.changePercent.toFixed(2)}%`,
      change: sector.changePercent,
      trend: sector.changePercent > 0 ? 'up' : sector.changePercent < 0 ? 'down' : 'flat',
      lastUpdate,
      source: 'Sektör Isı Haritası',
    });
  }
  for (const g of overview.topGainers.slice(0, 2)) {
    widgets.push({
      symbol: g.ticker,
      label: g.ticker,
      value: g.price.toLocaleString('tr-TR', { maximumFractionDigits: 2 }),
  ...
  for (const l of overview.topLosers.slice(0, 2)) {
    widgets.push({
      symbol: l.ticker,
      label: l.ticker,
      value: l.price.toLocaleString('tr-TR', { maximumFractionDigits: 2 }),
  ...
  for (const v of overview.volumeLeaders.slice(0, 2)) {
    widgets.push({
      symbol: v.ticker,
      label: v.ticker,
      value: v.volume.toLocaleString('tr-TR'),
      change: v.changePercent,
      trend: v.changePercent > 0 ? 'up' : v.changePercent < 0 ? 'down' : 'flat',
      lastUpdate,
      source: 'Hacim Liderleri',
    });
  }
  return widgets;
}
...
function statusTone(status: string): string {
  ...
  if (s.includes('healthy') || s.includes('running') || s.includes('completed'))
    return 'border-emerald-400/40 bg-emerald-400/10 text-emerald-200';
  if (s.includes('degraded') || s.includes('pending') || s.includes('queued') || s.includes('idle'))
    return 'border-amber-400/40 bg-amber-400/10 text-amber-200';
  if (s.includes('fail') || s.includes('critical') || s.includes('unhealthy'))
...
function getMarketStatus(now = new Date()) {
  ...
  const state = isWeekend
    ? 'Hafta Sonu'
    : now >= open && now <= close
      ? 'Açık'
      : now >= preMarket && now < open
        ? 'Seans Öncesi'
        : 'Kapalı';
  ...
  if (now >= close || isWeekend)
    nextOpening.setDate(now.getDate() + (day === 5 ? 3 : day === 6 ? 2 : day === 0 ? 1 : 1));
...
function mapProviderRows(rawProviders: Array<Record<string, unknown>>): ProviderRow[] {
  const rawByName = new Map(
    rawProviders.map((p) => [String(p.provider ?? p.name ?? '').toLowerCase(), p]),
  );
  return expectedProviders.map((providerName, index) => {
    const matched = [...rawByName.entries()].find(([name]) =>
      name.includes(providerName.toLowerCase()),
    );
    const raw = matched?.[1];
    ...
    return {
      ...
      circuitBreaker:
        consecutiveFailures > 2 ? 'Open' : consecutiveFailures > 0 ? 'Half-open' : 'Closed',
...
function MetricStrip({
  ...
  const healthyProviders = snapshot.providers.filter(
    (p) => p.status.toLowerCase() === 'healthy',
  ).length;
  const activeJobs = snapshot.workflowJobs.filter(
    (j) => j.status.toUpperCase() === 'RUNNING',
  ).length;
  ...
  return (
    <div className="flex flex-wrap items-center gap-2 rounded-lg border border-slate-700 bg-slate-950 px-3 py-2">
      {[
        ...
        [
          'Providers',
          `${healthyProviders}/${snapshot.providers.length || expectedProviders.length}`,
        ],
        ['Cache', `${cacheRate.toFixed(0)}%`],
        ['Macro', snapshot.macroScore == null ? 'Veri yok' : snapshot.macroScore.toFixed(0)],
      ].map(([label, value]) => (
...
function MarketWidgetGrid({ widgets }: { widgets: MarketWidget[] }) {
  if (widgets.length === 0) {
    return (
      <div className="flex h-40 items-center justify-center rounded-md border border-dashed border-slate-700 text-sm text-slate-500">
        Piyasa verisi yok — sağlayıcılardan fiyat alınamadı
      </div>
    );
  }
  return (
    <div className="grid grid-cols-2 gap-2 xl:grid-cols-5">
      {widgets.map((widget) => (
        <div
          key={`${widget.symbol}-${widget.source}`}
          className="rounded-md border border-slate-800 bg-slate-900/80 p-3 transition-colors hover:border-cyan-500/40"
        >
          ...
          <div
            className={cn(
              ...
              widget.change >= 0 ? 'text-emerald-300' : 'text-red-300',
            )}
          >
            {formatPercent(widget.change)}
          </div>
          ...
          {rows.length === 0 ? (
            <tr>
              <td colSpan={10} className="px-2 py-8 text-center text-slate-500">
                İzleme listesi sembolü döndürülmedi
              </td>
            </tr>
          ) : (
            rows.map((row) => (
              <tr
                key={row.ticker}
                className="border-b border-slate-900 transition-colors hover:bg-slate-900/80"
              >
                ...
                <td className="px-2 py-2 text-right font-mono text-slate-300">
                  {row.volume != null ? row.volume.toLocaleString('tr-TR') : '--'}
                </td>
                <td
                  className={cn('px-2 py-2 text-right font-mono', scoreTone(row.eliteScore ?? 0))}
                >
                  {row.eliteScore != null ? row.eliteScore.toFixed(0) : '--'}
                </td>
                <td
                  className={cn('px-2 py-2 text-right font-mono', scoreTone(row.macroScore ?? 0))}
                >
                  {row.macroScore != null ? row.macroScore.toFixed(0) : '--'}
                </td>
                <td className="px-2 py-2 text-right font-mono text-slate-300">
                ...
                <td className="px-2 py-2">
                  <span
                    className={cn('rounded border px-2 py-0.5 font-medium', statusTone(row.signal))}
                  >
                    {row.signal}
                  </span>
                </td>
  ...
  return (
    <div className="space-y-2">
      {providers.map((provider) => (
        <div
          key={provider.name}
          className="grid grid-cols-[1fr_auto] gap-2 rounded-md border border-slate-800 bg-slate-900/70 p-2"
        >
          <div>
            ...
            <div className="mt-1 text-[11px] text-slate-500">
              Önbellek {provider.cacheStatus} / DC {provider.circuitBreaker}
            </div>
          </div>
  ...
  return (
    <div className="rounded-md border border-slate-800 bg-slate-900/70 p-3">
      <div className="flex items-center justify-between">
        ...
        <span className={cn('rounded border px-2 py-0.5 text-xs', statusTone(status.state))}>
          {status.state}
        </span>
      </div>
...
export default function DashboardPage() {
  ...
  const fetchAll = useCallback(async () => {
    ...
    const [
      ...
      optionalSettle<{
        ...
        typeof sdkClient.earlyOpportunities === 'function'
          ? () => sdkClient.earlyOpportunities(15)
          : undefined,
      ),
      optionalSettle<Awaited<ReturnType<typeof sdkClient.marketOverview>>>(
        sdkClient.marketOverview,
      ),
    ]);

    const scanData = (scanRes.data as Record<string, unknown> | null) ?? {};
    const scanCandidates = (scanData['topCandidates'] ?? scanData['sonuclar'] ?? []) as Array<
      Record<string, unknown>
    >;
    const oppData = (oppRes.data as Record<string, unknown> | null) ?? {};
    const oppDataInner = (oppData['data'] as Record<string, unknown> | null | undefined) ?? {};
    const oppCandidates = (oppDataInner['items'] ?? oppData['sonuclar'] ?? []) as Array<
      Record<string, unknown>
    >;

    const toScanRow = (c: Record<string, unknown>): ScanResultRow => ({
      symbol: String(c.symbol ?? c.ticker ?? ''),
      score: toNumber(c.compositeScore ?? c.eliteScore ?? c.aiScore ?? c.opportunityScore ?? 0),
      opportunityScore: toNumber(c.opportunityScore ?? c.compositeScore ?? c.eliteScore ?? 0),
      rank: toNumber(c.rank, 0),
      status: String(c.status ?? ''),
      reason:
        Array.isArray(c.reasons) && c.reasons.length > 0 ? String(c.reasons[0]) : 'Pipeline signal',
    });

    const scannerResults = scanCandidates.map(toScanRow);
    const candidateRows = oppCandidates.map(toScanRow);

    const intelDecisions = new Map<
      string,
      {
        decisionScore: number;
        decisionStatus: string;
    ...
    const opportunities = candidateRows.map((c) => {
    ...
    const macroData = macroRes.data as Record<string, unknown> | null;
    const macroScore = toNullableNumber(
      (macroData?.['snapshot'] as Record<string, unknown> | null | undefined)?.['macroScore'] ??
        macroData?.['score'] ??
        macroData?.['macroScore'],
    );
    const symbols = [
      ...new Set(
        (watchRes.data?.data?.lists ?? []).flatMap((l) =>
          ((l.entries as Array<{ symbol?: string }> | undefined) ?? [])
    ...
    const sourceRows =
      symbols.length > 0
        ? symbols.slice(0, 8)
        : scannerResults.slice(0, 8).map((row) => row.symbol);
    const nowLabel = new Date().toLocaleTimeString('tr-TR', { hour: '2-digit', minute: '2-digit' });

    const quoteRows = await Promise.all(
      sourceRows.map((ticker) =>
        typeof sdkClient.marketData === 'function'
          ? settle<Awaited<ReturnType<typeof sdkClient.marketData>>>(sdkClient.marketData(ticker))
          : Promise.resolve<SettleResult<Awaited<ReturnType<typeof sdkClient.marketData>>>>({
              data: null,
    ...
    const quoteByTicker = new Map(
      sourceRows.map((ticker, index) => [ticker, quoteRows[index].data]),
    );

    const watchlist: WatchlistRow[] = sourceRows.map((ticker) => {
      const scan =
        scannerResults.find((row) => row.symbol === ticker) ??
        opportunities.find((row) => row.symbol === ticker);
      const quote = quoteByTicker.get(ticker);
      const eliteScore = scan?.score ?? null;
      ...
      return {
        ...
        macroScore,
        confidence:
          quote && scan?.score != null ? Math.min(99, Math.max(1, scan.score * 0.85)) : null,
        signal:
          eliteScore == null
            ? 'Veri yok'
            : eliteScore >= 80
          ...
          changePercent == null
            ...
            : changePercent > 1
              ...
              : changePercent < -1
                ? 'Düşüş'
                : 'Yatay',
        lastUpdate: quote?.timestamp ? toDateLabel(quote.timestamp) : nowLabel,
      };
    });

    const overview = (overviewRes.data as MarketOverview | null) ?? null;
    const chartSymbol = sourceRows[0] ?? null;
    const historyRes =
      chartSymbol && typeof sdkClient.marketDataHistorical === 'function'
        ? await settle<Awaited<ReturnType<typeof sdkClient.marketDataHistorical>>>(
            sdkClient.marketDataHistorical(chartSymbol, '1d'),
          )
        : { data: null, error: 'Data unavailable' };
    const chartData = (historyRes.data?.data ?? []).slice(-30).map((point) => ({
      time: toDateLabel(point.timestamp),
      value: toNumber(point.close),
    }));

    setSnapshot({
      scannerResults,
      scannerTotal: toNumber(
        scanData['toplamHisse'] ??
          (scanData['statistics'] as Record<string, unknown> | null | undefined)?.['totalSymbols'],
        scannerResults.length,
      ),
      opportunities,
      workflowJobs: (wfRes.data?.data?.jobs ?? []).map((j) => ({
        id: j.id,
      ...
      performance: perfRes.data
        ? {
            metrics: perfRes.data.data?.metrics || [],
            system: perfRes.data.data?.system || {
              uptimeMs: 0,
              memoryUsageBytes: 0,
              cpuUsagePercent: 0,
            },
            cache: perfRes.data.data?.cache,
          }
        : null,
      events: (evtRes.data?.data?.events ?? []).map((e) => ({
        id: e.id,
        type: e.type,
      ...
      diagnostics: (diagRes.data?.components ?? []).map((c) => ({
        name: c.name,
        status: c.status,
        message: c.message,
        duration: c.duration,
      ...
      watchlist,
      marketWidgets: toMarketWidgets(overview, macroScore),
      macroScore,
    ...
    if (errors > 0)
      setLoadError(`${errors} data channel${errors > 1 ? 's' : ''} could not be loaded`);
    setRefreshing(false);
  }, []);
  ...
  const apiLatencyMetrics = (snapshot.performance?.metrics || []).filter(
    (m) => m.category === 'api_response',
  );
  const apiLatency =
    apiLatencyMetrics.length > 0
      ? apiLatencyMetrics.reduce((sum, metric) => sum + metric.avg, 0) / apiLatencyMetrics.length
      : 0;
  const completedSteps = activeWorkflow
    ? snapshot.workflowJobs.filter((job) => job.status.toUpperCase() === 'COMPLETED').length
    : 0;
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <div className="text-[11px] uppercase tracking-[0.28em] text-cyan-300">
            Profesyonel AI Terminali
          </div>
          <h2 className="mt-1 text-xl font-semibold tracking-tight text-slate-50">
            BIST Elite AI Komuta Merkezi
          </h2>
        </div>
        {loadError && (
          <span className="rounded border border-amber-400/40 bg-amber-400/10 px-3 py-1 text-xs text-amber-200">
            {loadError}
          </span>
        )}
      </div>
      ...
      <div className="grid min-h-[720px] grid-cols-1 gap-3 xl:grid-cols-[minmax(230px,0.8fr)_minmax(520px,2fr)_minmax(280px,0.95fr)]">
        <div className="space-y-3">
                ...
                >
                  <div>
                    ...
                    <div className="text-[11px] text-slate-500">
                      {row.signal} / {row.trend}
                    </div>
                  </div>
                  <div className="text-right">
                    <div className={cn('font-mono text-sm', scoreTone(row.eliteScore ?? 0))}>
                      {row.eliteScore == null ? '--' : row.eliteScore.toFixed(0)}
                    </div>
                    <div
                      className={cn(
              ...
              {snapshot.scannerResults.slice(0, 6).map((row) => (
                ...
                >
                  <div className="flex items-center justify-between">
                    ...
                    <span className={cn('font-mono', scoreTone(row.score))}>
                      {row.score.toFixed(0)}
                    </span>
                  </div>
          ...
          <Panel title="Portföy" icon={Briefcase} defaultCollapsed resizable>
            <Link className="text-sm text-cyan-200 hover:text-cyan-100" to="/portfolio">
              Portföy istihbaratını aç
            </Link>
          </Panel>
          ...
          <Panel title="Alarmlar" icon={Bell} resizable>
            <div className="space-y-2">
              {liveEvents
                .filter((event) => event.category === 'alerts')
                .slice(0, 4)
                .map((event) => (
                  <div
                    key={event.id}
                    className="rounded border border-amber-400/30 bg-amber-400/10 p-2 text-xs text-amber-100"
                  >
                    {event.type}
                  </div>
                ))}
              {liveEvents.filter((event) => event.category === 'alerts').length === 0 && (
                <div className="text-sm text-slate-500">Aktif alarm yok</div>
              )}
            </div>
          </Panel>
        </div>
            ...
            <div className="h-72">
              {snapshot.chartData.length === 0 ? (
                <div className="flex h-full items-center justify-center text-sm text-slate-500">
                  Grafik verisi yok — seçili sembol için geçmiş veri bulunamadı
                </div>
              ) : (
            ...
            <Panel title="Şirket Özeti" icon={Database} resizable>
              <div className="space-y-3 text-sm">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span className="font-mono">
                    {selected?.volume != null ? selected.volume.toLocaleString('tr-TR') : '-'}
                  </span>
                </div>
                <div className="flex items-center justify-between">
                  ...
                  <span>
                    {snapshot.providers.find(
                      (provider) => provider.status.toLowerCase() === 'healthy',
                    )?.name ?? "Mevcut API'ler"}
                  </span>
                </div>
              </div>
              ...
              <div className="space-y-3 text-sm">
                <div className="rounded-md border border-slate-800 bg-slate-900/70 p-3">
                  ...
                  <div className="mt-1 text-lg font-semibold text-cyan-200">
                    {selected?.signal ?? 'Veri bekleniyor'}
                  </div>
                  <p className="mt-2 text-xs leading-5 text-slate-400">
                    {selected
                      ? `${selected.ticker}${selected.eliteScore != null ? `, ${selected.eliteScore.toFixed(0)} Elite Skor` : ''}${selected.macroScore != null ? `, ${selected.macroScore.toFixed(0)} Makro Skor` : ''}${selected.confidence != null ? ` ve %${selected.confidence.toFixed(0)} güven` : ''}${selected.eliteScore != null || selected.macroScore != null ? ' düzeyini birleştirir.' : ' için doğrulanmış skor bulunamadı.'}`
                      : "Bu panel, tarayıcı ve izleme listesi API'lerinden doldurulur."}
                  </p>
                </div>
              </div>
            </Panel>
        ...
        <div className="space-y-3">
          <Panel title="Elite Skor" icon={Gauge} resizable>
            <div className={cn('font-mono text-5xl', scoreTone(selected?.eliteScore ?? 0))}>
              {selected?.eliteScore != null ? selected.eliteScore.toFixed(0) : '--'}
            </div>
          ...
          <Panel title="Makro Skor" icon={Activity} resizable>
            <div className={cn('font-mono text-5xl', scoreTone(snapshot.macroScore ?? 0))}>
              {snapshot.macroScore != null ? snapshot.macroScore.toFixed(0) : 'Veri yok'}
            </div>
            <div className="mt-2 text-xs text-slate-500">Makro istihbarat uç noktası</div>
          </Panel>

          <Panel title="Güven" icon={ShieldCheck} resizable>
            <div className={cn('font-mono text-5xl', scoreTone(selected?.confidence ?? 0))}>
              {selected?.confidence != null ? `${selected.confidence.toFixed(0)}%` : '--'}
            </div>
          ...
          <Panel title="İş Hattı Durumu" icon={ListChecks} resizable>
            <div className="space-y-2 text-sm">
              <div className="flex justify-between gap-3">
                <span className="text-slate-500">Mevcut Adım</span>
                <span className="text-right">{activeWorkflow?.status ?? 'Boşta'}</span>
              </div>
              <div className="flex justify-between gap-3">
                <span className="text-slate-500">Tamamlanan Adımlar</span>
                <span className="font-mono">{completedSteps}</span>
              </div>
              <div className="flex justify-between gap-3">
                <span className="text-slate-500">Çalışma Süresi</span>
                <span className="font-mono">{apiLatency.toFixed(0)} ms</span>
              </div>
              <div className="flex justify-between gap-3">
                <span className="text-slate-500">Kullanılan Sağlayıcı</span>
                <span>{snapshot.providers[0]?.name ?? 'Veri yok'}</span>
              </div>
              <div className="flex justify-between gap-3">
                <span className="text-slate-500">Zamanlayıcı Görevi</span>
                <span>{snapshot.schedulerJobs[0]?.jobName ?? 'Aktif görev yok'}</span>
              </div>
            </div>
            <Link
              to="/pipeline-status"
        ...
        <Panel title="Canlı Kayıtlar" icon={Radio} resizable>
          <div className="max-h-56 space-y-2 overflow-y-auto pr-1 scrollbar-thin">
            {snapshot.events.slice(0, 8).map((event) => (
              <div
                key={event.id}
                className="rounded border border-slate-800 bg-slate-900/70 p-2 font-mono text-xs text-slate-300"
              >
        ...
        <Panel title="WebSocket Olayları" icon={Radio} resizable>
          <div className="max-h-56 space-y-2 overflow-y-auto pr-1 scrollbar-thin">
            {websocketEvents.length === 0 ? (
              <div className="text-sm text-slate-500">WebSocket olayları bekleniyor</div>
            ) : (
              websocketEvents.slice(0, 8).map((event) => (
                <div
                  key={event.id}
                  className="rounded border border-cyan-500/20 bg-cyan-500/10 p-2 font-mono text-xs text-cyan-100"
                >
            ...
            {snapshot.schedulerJobs.map((job) => (
              ...
              >
                <div className="flex items-center justify-between gap-3">
                  ...
                  <span className={cn('rounded border px-1.5 py-0.5', statusTone(job.status))}>
                    {job.status}
                  </span>
                </div>
                <div className="mt-1 text-slate-500">
                  Son çalışma: {toDateLabel(job.lastExecution?.startedAt)}
                </div>
              </div>
            ))}
            {snapshot.schedulerJobs.length === 0 && (
              <div className="text-sm text-slate-500">Zamanlayıcı etkinliği yok</div>
            )}
          </div>
        </Panel>
      </div>

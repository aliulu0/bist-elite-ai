...
class ADLPlugin(BaseVolumePlugin):
    ...
    def calculate(self, prices: list[PriceBar], **params) -> IndicatorResult:
        ...
        adl_values: list[float | None] = [None] * n

        if n == 0:
            return IndicatorResult(
                indicator=self.display_name, parameters={}, values=adl_values, dates=dates,
            )
        ...
        for i in range(1, n):
            ...
            if rng > 0:
                ...
                cfm = 0.0
            adl_values[i] = (adl_values[i - 1] or 0.0) + cfm * prices[i].volume

        current = adl_values[-1]
        previous = adl_values[-2] if n >= 2 else None
